- [定时器中断结构图](#定时器中断结构图)
- [定时器中断代码初始化顺序](#定时器中断代码初始化顺序)
- [AFIO重映射引脚](#afio重映射引脚)
- [定时器输出PWM](#定时器输出pwm)
- [输入捕获](#输入捕获)
- [定时器的相关中断](#定时器的相关中断)
- [编码器接口模式](#编码器接口模式)

### 定时器中断结构图
![images](images/image.png)
### 定时器中断代码初始化顺序
现在假设使用外部时钟, 详情见 system/Timer.c:
1. 通过GPIO接入外部时钟, 初始化GPIO的RCC及引脚
2. 使用定时器, 先初始化RCC_APB1PeriphClockCmd
3. 配置使用外部时钟还是内部时钟
4. 配置时基单元, 包括预分频PSC, 自动重装ARR
5. 使能TIM中断, TIM_ITConfig()
6. 配置NVIC, 使能TIM2_IRQn时钟类型的中断
7. 使能定时器, TIM_Cmd()
8. 编写中断函数TIM2_IRQHandler()


### AFIO重映射引脚
使用函数GPIO_PinRemapConfig()

### 定时器输出PWM
内部时钟, 经过预分频PSC, 然后CNT计数器开始计数, 每次计数到ARR(自动重装载寄存器), 就会重新从0开始计数(向上计数模式, 还有比如递减的).
为什么可以输出PWM波形, 是因为还有一个捕获比较寄存器CCR, CNT每次都会与CCR比较, 基于不同的模式, 比如当CNT小于CCR, 则输出高电平, 否则输出低电平, 当CNT到达ARR就是下一周期开始.
所以: (PSC和ARR记得代码填参数-1, 因为0就是一倍频)
* PWM频率= 72MHz/PSC/ARR
* PWM分辨率= 1/ARR 也就是占空比最小可以调节多少, 比如占空比一次最小增加 0.1%, 那么分辨率就是 0.1%, 那么ARR必须等于1000.
* PWM占空比= CCR/ARR 通过调节CCR的值, 可以动态调整占空比

### 输入捕获
好文: https://blog.csdn.net/qq_38410730/article/details/80011330


### 定时器的相关中断
1. UIF：更新中断标记
   * 比如CNT向上计数, 到达ARR, 触发UEV(更新事件: CNT重新计数),就会触发中断, 然后CNT清零, 然后重新计数.
   * 软件设置事件产生寄存器TIMx_EGR的UG位, 也可以触发UEV事件, 产生中断
2. CCxIF: 捕获/比较x 中断标记
   * 比较输出模式时, 比如输出PWM波形, CNT的值与CCR的值相等时, 触发中断
   * 捕获输入模式时, 当外部信号通过数字滤波,边沿检测成功,分频器检测指定次数后, CNT的值传送到CCR(捕获)时, 会触发中断.

### 编码器接口模式
编码器接口模式基本上相当于使用了一个带有方向选择的外部时钟. 有两个输入, 只能是CH1和CH2.
方向: 在每个输入信号的边沿会使得CNT增加或者减小(两个输入的PWM波形的相位不同), 如果ARR=4-1, 那么 CNT=4=0, 也就是增加到3的时候, 再增就为0, CNT=-1=ARR, 也就是0再减就到了变成ARR了.  
速度: 通过设置另一个TIM的每隔一段时间的定时中断, 获取CNT的同时清零CNT, 就可以计算出速度


