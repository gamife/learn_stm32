...
void PWM_Init(void){
    ...
    // 与ARR一起控制PWM的输出频率, 7MHz/720/100 = 1Khz, 所以PWM的输出频率是 1KHz
    TIM_TimeBaseStructure.TIM_Prescaler = 720 -1;   // PSC 预分频器, 比如内部时钟10MHz, 预分频(2-1)二分频,0表示1分频, 也就是内部时钟两个周期, 而TIM.CNT只会记一次数
    TIM_TimeBaseStructure.TIM_ClockDivision = TIM_CKD_DIV1; // /设置时钟分割:TDTS, 与输入的数字滤波相关.
    TIM_TimeBaseStructure.TIM_CounterMode = TIM_CounterMode_Up;
    TIM_TimeBaseStructure.TIM_RepetitionCounter = 0;
    TIM_TimeBaseInit(TIM2, &TIM_TimeBaseStructure);
    // TIM_ClearFlag(TIM2, TIM_FLAG_Update);
...
/**
  * @brief  设置TIM2_CH1 (PA0) 输出PWM波形的占空比
  * @param  Compare1: CCR的值, 范围 0~100
  * @retval None
  */
...
/**
  * @brief  设置TIM2_CH1 (PA0) 输出PWM波形的频率, 也就是预分频器PSC的值.
  *     输出频率: 72Mhz/100/PSC
  * @param  Prescaler: 预分频器PSC的值
  * @retval None
  */
